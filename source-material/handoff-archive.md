...
(`<prod-db-id>`), verified this session from his screenshots:**
...
Datasource resolved to `<staging-db-id>` (confirmed correct
staging host, never `<prod-db-id>` prod) - "Database schema is
...
`<staging-db-name>`/`<staging-db-id>` host, same as L1 - never prod)
...
- **Live-tested both features directly** (Playwright browser, since reading
  ...
  smoke credentials (`<smoke-user>` / `<smoke-pass>`) returned 401 — gone,
...
- **Migration applied to STAGING, done by Seth manually (browser agent,
  RUNBOOK "Schema-change deploy"):** confirmed host `<staging-db-name>` /
  `<staging-db-id>` throughout, never touched
  `<prod-db-name>` / `<prod-db-id>` (prod). `ALTER TABLE
...
- **U7 visual sign-off PENDING** — Seth smokes the Vercel build of
  `analytics-engine` @ `f22989d` (login `<smoke-user>`, band on Home, palettes x
...
- **Staging DB was reset** by tonight's full `npm test` run (expected pretest
  ...
  account: `<smoke-user>` / `<smoke-pass>` (email `<smoke-user>@example.com`),
  ...
  **SUPERSEDED (July 3):** `<smoke-user>` gone in a later reset. Current smoke
  account: username `<smoke-user>` (UNDERSCORE, not hyphen - Cursor created the
  account first and usernames are immutable) / `<smoke-pass>` (email
