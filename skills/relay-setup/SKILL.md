---
name: relay-setup
description: Install or upgrade this workflow in a project - fill the setup manifest, build a check command that can fail, generate the agent contract and task queue, render the per-tool adapters, and run the first lap. Use when someone asks to set up, install, adopt, or upgrade this workflow.
---
...
- **`references/check-lane.md`** - building a check command that can
  actually fail, and proving it (question P2/P2a, step 6).
- **`references/upgrading.md`** - upgrading an install that already
  exists without destroying local edits, and re-verifying claims that
  rot.

**Already installed? This is also the upgrade path** - go to
`references/upgrading.md` instead of re-running the install.

**Done when:** the project holds a filled manifest, a generated agent
contract, a state file, a task queue with one starter task, adapters for
...
- **P2. Check command** - the one command that proves the project still
  works, re-runnable fresh in under ~2 minutes. **This is the answer worth
  real effort** - it is what every review re-runs forever.
  *Default: name what exists, and BUILD A MINIMAL ONE if nothing does.*
  Don't record an absence and move on. Decide the command now; step 6
  builds it through the loop. **How to pick one and how to prove it:
  `references/check-lane.md`.**
- **P2a. Falsifiability** - anything this setup creates is proven RED
  before it is accepted: break something trivial, run it, watch it fail,
  revert, watch it pass. *Default: ON, not deletable for commands setup
  creates.* A check nobody has watched fail is an unverified claim, and
...
- **M2. Escalation triggers.** *Default:* schema/data-model design;
  ...
  can't close; delivered work and spec disagreeing in a way the task file
  doesn't settle.

### Extras

- **V1. Visual skill map** - a single self-contained HTML page showing
  every skill installed here and when each one fires, regenerated by
  `skill-map` whenever the set changes. *Default: ask.*
  **This is the one question to ask out loud rather than infer**, because
  it's a matter of taste rather than evidence, and it costs nothing to
  skip. Put it plainly: *"Want a visual page showing your skills and when
  each one kicks in? It's one HTML file you can open in a browser -
  entirely optional."* Record the answer either way; `skill-map` reads it
  and never generates the page unprompted.
...
- **`<S3>`** with a `BLOCK.md` describing the task format, and
  **`<S3>/u0-hello-relay.md`**, the starter task for step 6.
- A `.gitignore` entry for `DELIVERY.md`.
- **If V1 is on**, the visual skill map - generated by `skill-map` after
  the adapters are rendered, so it reflects what actually landed.
...
The five skills are `relay-setup`, `relay-plan`, `relay-execute`,
`relay-review`, and `skill-map`. Skills are an open standard, so most
tools read the folder directly; the adapter is only about where each tool
looks for them.

| Tool | Render |
|---|---|
| Claude Code | `CLAUDE.md` importing `AGENTS.md`; the skills into `.claude/skills/`; gate items as permission settings |
| Cursor | `.cursor/rules/` pointing at `AGENTS.md`, skills in the skills directory |
